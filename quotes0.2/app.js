...
function toCardModel(row) {
  return {
    id: String(row.id),
    text: row.text,
    author: row.author || '',
    lines: wrapTextToWidth(row.text, Math.max(20, row.size - CARD_PADDING * 2)),
    size: row.size,
    bg: row.bg,
    fg: row.fg,
    x: row.x,
...
async function loadCardsFromDB() {
  ...
  for (const row of data) {
    cards.push(toCardModel(row));
  }
  render();
}

function subscribeCardsRealtime() {
  if (!supabaseClient) return;
...
async function addCard() {
  ...
  if (supabaseClient) {
    const { data, error } = await supabaseClient.from('cards').insert(row).select().single();
    if (error) {
      console.error(error);
      return;
    }
...
async function bootstrap() {
  if (supabaseClient) {
    ...
    defaultSeedCards.forEach((item, index) => addSeedCard(item, index));
  }
